...
async def multi_agent_interaction_with_agent_outputs(query: str) -> tuple[str, dict[str, str]]:
    """Run the retail multi-agent workflow and return both the final output and per-agent streamed text."""
    final_output = ""
    ...
    async with (
        ...
        async for event in events:
            if isinstance(event, AgentRunUpdateEvent):
                eid = event.executor_id
                agent_outputs[eid] = agent_outputs.get(eid, "") + (event.data or "")
            elif isinstance(event, WorkflowOutputEvent):
...
if __name__ == "__main__":
    ...
    query = "Can you provide me with a detailed planogram for a retail customer CUST-1003 store aisle focused on electronics, including product placements based on inventory levels and sales trends?"

...
def redteam_main() -> None:
    start_time = time.time()
    print(f"Starting red team evaluation at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}")
    ...
    with (
        ...
        print(f"\n{'='*60}")
        print(f"Evaluation completed at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}")
